...
class Ant(BaseAgent):
    ...
    def draw(self, surface: pygame.Surface, is_best: bool = False,
             ...
             trail_surface: pygame.Surface = None):
        """
        ...
        trail_surface  : Optional SRCALPHA surface to draw trails onto. When
                         provided, trails are rendered with per-pixel alpha so
                         they appear semi-transparent against any background.
        """
        if not self._trail:
            return
        ...
        # --- Arrowhead polygon ------------------------------------------------
        # Derive the travel direction using math.atan2 on the velocity vector
        # (last displacement in the trail). Fall back to _heading when the trail
        # has fewer than two committed positions.
        cx = int(self._position[0])
        cy = int(self._position[1])

        if len(self._trail) >= 2:
            vx = self._trail[-1][0] - self._trail[-2][0]
            vy = self._trail[-1][1] - self._trail[-2][1]
        ...
        # Arrowhead geometry: nose tip forward, two swept-back wing tips,
        # and a shallow tail indent — forms a concave chevron / dart shape.
        nose_len    = config.ANT_RADIUS * 2.2
        wing_len    = config.ANT_RADIUS * 1.6
        wing_angle  = 2.45   # ~140° off the nose axis for swept wings
        tail_indent = config.ANT_RADIUS * 0.6

        nose  = (cx + math.cos(angle_rad) * nose_len,
                 cy + math.sin(angle_rad) * nose_len)
        left  = (cx + math.cos(angle_rad + wing_angle) * wing_len,
                 cy + math.sin(angle_rad + wing_angle) * wing_len)
        right = (cx + math.cos(angle_rad - wing_angle) * wing_len,
                 cy + math.sin(angle_rad - wing_angle) * wing_len)
        tail  = (cx - math.cos(angle_rad) * tail_indent,
                 cy - math.sin(angle_rad) * tail_indent)

        # Only draw arrowhead for navigating/visible ants; dead non-winners
        # render as a tiny dim dot so they don't clutter the scene.
        if self._is_alive or is_best or self._has_won or active_uniform:
            ...
            # Dead non-winner: minimal 2-px dot in muted colour
            pygame.draw.circle(surface, body_color, (cx, cy), 2)
