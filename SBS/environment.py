...
class StartZone(BaseZone):
    ...
    def draw(self, surface: pygame.Surface):
        ...
        pygame.draw.circle(surface, (0, 80, 35), (self._x, self._y),
                           self._radius + 5)
        pygame.draw.circle(surface, self._color, (self._x, self._y), self._radius)
        # Inner bright ring for neon edge highlight
        pygame.draw.circle(surface, (180, 255, 200), (self._x, self._y),
                           self._radius, 2)
...
class TargetZone(BaseZone):
    ...
    def draw(self, surface: pygame.Surface):
        ...
        # Outer glow halo pulsing with the zone
        pygame.draw.circle(surface, (90, 70, 0),
                           (self._x, self._y), self._radius + pulse + 7)
        pygame.draw.circle(surface, self._color,
                           (self._x, self._y), self._radius + pulse)
        # Bright inner highlight ring
        pygame.draw.circle(surface, (255, 248, 190),
                           (self._x, self._y),
...
class Wall:
    ...
    def draw(self, surface: pygame.Surface):
        pygame.draw.rect(surface, config.COLOR_WALL, self._rect, border_radius=10)
        # Top-edge highlight gives a subtle depth impression
